...
class inner_GNN(torch.nn.Module):
    def __init__(self, num_node_features, num_genes, device):
        ...
        #torch.nn.init.xavier_uniform_(self.conv_first)

        self.bn1 = BatchNorm(self.n_hidden)
        
        #self.conv1 = GATv2Conv(self.n_hidden, self.n_hidden)
        #self.bn3 = BatchNorm(self.n_hidden)
        
        self.conv_last = GATv2Conv(self.n_hidden, 1,weight_initializer = None)
        self.bn2 = BatchNorm(1)
        
        self.device = device
        #self.linear = torch.nn.Linear(self.num_genes*self.n_hidden, self.num_classes)
